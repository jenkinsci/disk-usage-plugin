...
public class ProjectDiskUsage implements Saveable{
    ...
    protected Map<String,Map<String,Long>> agentWorkspacesUsage = new ConcurrentHashMap<String,Map<String,Long>>();
    ...
    private Long cashedDiskUsageNonAgentWorkspace = 0L;
    ...
    public void removeNode(Node node){
        agentWorkspacesUsage.remove(node.getDisplayName());
    }

    public Map<String,Map<String,Long>> getAgentWorkspacesUsage(){
        return Maps.newHashMap(agentWorkspacesUsage);
    }
    ...
    public void putAgentWorkspaceSize(Node node, String path, Long size){
        Map<String,Long> workspacesInfo = agentWorkspacesUsage.get(node.getNodeName());
        ...
        agentWorkspacesUsage.put(node.getNodeName(), workspacesInfo);
    }
...
//        removeDeletedBuilds();
    ...
    public void addBuildInformation(DiskUsageBuildInformation info, AbstractBuild build, Long size){
        if(!containsBuildWithId(info.getId())){
            ...
            if(build!=null && build.getWorkspace()!=null){
                ...
                if(exists) {
                    putAgentWorkspaceSize(build.getBuiltOn(), build.getWorkspace().getRemote(), size);
                }
            }
        }
        if(notLoadedBuilds.containsKey(info.getId()) || notLoadedBuilds.containsKey(info.getOldId())){
    ...
    /**
     * @return the cashedDiskUsageNonAgentWorkspace
     */
    public Long getCashedDiskUsageNonAgentWorkspace() {
        return cashedDiskUsageNonAgentWorkspace;
    }

    /**
     * @param cashedDiskUsageNonAgentWorkspace the cashedDiskUsageNonAgentWorkspace to set
     */
    public void setCashedDiskUsageNonAgentWorkspace(Long cashedDiskUsageNonAgentWorkspace) {
        this.cashedDiskUsageNonAgentWorkspace = cashedDiskUsageNonAgentWorkspace;
    }
